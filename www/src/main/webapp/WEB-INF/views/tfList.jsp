			...
			<ul class="pagination justify-content-center">
				<c:if test="${pagevo.prev }">
					<li class="page-item"><a class="page-link"
						href="tList?page=${pagevo.startPage -1 }">이전</a></li>
				</c:if>
				<c:forEach begin="${pagevo.startPage }" end="${pagevo.endPage }"
					var="idx">
					<li class="page-item"><a class="page-link"
						href="tList?page=${idx}">${idx}</a></li>
				</c:forEach>
				<c:if test="${pagevo.next }">
					<li class="page-item"><a class="page-link"
						href="tList?page=${pagevo.endPage +1 }">다음</a></li>
				</c:if>
			</ul>
		</nav>
		<div class="container-md">
			...
			<button type="button" class="btn btn-primary" data-bs-toggle="modal"
				data-bs-target="#exampleModal">맛집 등록</button>
		</div>
	</div>
	<!-- 등록 Modal -->
		...
		<div class="modal-dialog">
			<div class="modal-content rounded-4 shadow">
				<div class="modal-header p-5 pb-4 border-bottom-0">
					<h1 class="fw-bold mb-0 fs-2">맛집 등록</h1>
					...
					<div class="modal-body p-5 pt-0">
						<div class="form-floating mb-3">
							<input type="text" class="form-control rounded-3"
								id="floatingInput" name="username" required> <label
								for="floatingInput">작성자</label>
						</div>
						<div class="form-floating mb-3">
							<input type=text class="form-control rounded-3"
								id="floatingInput" name="place" required> <label
								for="floatingInput">장소</label>
						</div>
						<div class="form-floating mb-3">
							<input type="text" class="form-control rounded-3"
								id="floatingInput" name="restaurant" required> <label
								for="floatingInput">맛집</label>
						</div>
						<div class="form-floating mb-3">
							<input type="text" class="form-control rounded-3"
								id="floatingInput" name="review" required> <label
								for="floatingInput">후기</label>
						</div>
						<div>
							<div class="mb-3">
								<label for="formFileSm" class="form-label">첨부파일</label> <input
